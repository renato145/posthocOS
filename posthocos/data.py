...
class VolumeItemList(ItemList):
    ...
    def from_paths(cls, path, metadata_path, tfm_params=None, **kwargs):
        '''
        Use ex:
        data = (scans.VolumeItemList.from_paths(path / 'train_scans128', path/'train_metadata.csv')
...
class SegmentationItemList(VolumeItemList):
    ...
    def from_paths(cls, path, path_masks, metadata_path=None, tfm_params=None):
        '''
        Use ex:
        data = (scans.SegmentationItemList.from_paths(path/'train_scans128', path/'train_segmentation', path/'train_metadata.csv')
                                          .split_by_rand_pct(0.2, seed=7)
                                          .label_from_bcolz()
...
class ParallelItemList(SegmentationItemList):
    ...
    def from_paths(cls, path, path_masks, metadata_path, tfm_params=None):
        '''
        Use ex:
        data = (scans.ParallelItemList.from_paths(path/'train_scans128', path/'train_segmentation', path/'train_metadata.csv')
                                      .split_by_rand_pct(0.2, seed=7)
                                      ...
                                      .normalize())
        '''
        volumes = bcolz.open(path, mode='r')
        masks = None if path_masks is None else bcolz.open(path_masks, mode='r')
        metadata = pd.read_csv(metadata_path)
        return cls.from_carray(volumes, masks, metadata, tfm_params)
