...
class Volume(ItemBase):
    ...
    def interpolate(self, shape:Collection[int]):
        shape = listify(shape, 3)
        d = F.interpolate(self.data[None], shape, mode='nearest')[0]
        return self.__class__(d, self.idx, self.metadata, self.tfm_params)
    ...
    for i,ax in zip(slices, axes):
        ...
        if title:
            ...
            for i,j in zip(extra_lbl,extra_inf):
                if isinstance(j, (float, np.float32)): t+= f'{i}: {j:.{precision}}'
                else                                 : t+= f'{i}: {j}'
